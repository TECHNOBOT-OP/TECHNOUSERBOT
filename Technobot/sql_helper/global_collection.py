...
class Techno_GlobalCollection(BASE):
    __tablename__ = "techno_globalcollection"
    ...
    def __repr__(self):
        return "<Techno Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Techno_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Techno_GlobalCollection.__table__.create(checkfirst=True)

TECHNO_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with TECHNO_GLOBALCOLLECTION:
        keyword_items = Techno_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with TECHNO_GLOBALCOLLECTION:
        if keyword_items := SESSION.query(Techno_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with TECHNO_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Techno_GlobalCollection.keywoard)
            .filter(Techno_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Techno_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Techno_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Techno_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Techno_GlobalCollection).all()
        for x in all_groups:
