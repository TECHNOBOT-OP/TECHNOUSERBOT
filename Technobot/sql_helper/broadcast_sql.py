...
class TechnoBroadcast(BASE):
    __tablename__ = "technobroadcast"
    ...
    def __repr__(self):
        return "<Techno Broadcast channels '%s' for %s>" % (
            self.group_id,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, TechnoBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


TechnoBroadcast.__table__.create(checkfirst=True)

TECHNOBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with TECHNOBROADCAST_INSERTION_LOCK:
        broadcast_group = TechnoBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with TECHNOBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(TechnoBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with TECHNOBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(TechnoBroadcast.keywoard)
            .filter(TechnoBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(TechnoBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(TechnoBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(TechnoBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(TechnoBroadcast).all()
        for x in all_groups:
