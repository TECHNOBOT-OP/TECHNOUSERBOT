...
class TechnoGloballist(BASE):
    __tablename__ = "technoglobal_list"
    ...
    def __repr__(self):
        return "<Techno global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, TechnoGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


TechnoGloballist.__table__.create(checkfirst=True)

TECHNOGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with TECHNOGLOBALLIST_INSERTION_LOCK:
        broadcast_group = TechnoGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with TECHNOGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(TechnoGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with TECHNOGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(TechnoGloballist.keywoard)
            .filter(TechnoGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(TechnoGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(TechnoGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(TechnoGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(TechnoGloballist).all()
        for x in all_groups:
