    ...
    if vname in vlist:
        ...
        if cmd == "set":
            if vname == "DEFAULT_USER":
                ...
                USERINFO = await techno.get_entity(techno.uid)
                FULL_USERINFO = (await techno(GetFullUserRequest(techno.uid))).full_user
                ...
                if FULL_USERINFO.about:
                    ...
                    delgvar("DEFAULT_BIO")
                try:
                    photos = await techno.get_profile_photos(techno.uid)
                    myphoto = await techno.download_media(photos[0])
...
@techno.techno_cmd(
    ...
    info={
        "header": "To customize your TechnoUserBot.",
        "options": {
...
async def custom_TechnoUserBot(event):
    "To customize your TechnoUserBot."
    reply = await event.get_reply_message()
    text = None
    if reply:
        text = reply.text
