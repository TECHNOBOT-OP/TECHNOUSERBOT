...
async def _(event):
    ...
    technoevent = await event.get_reply_message() if event.reply_to_msg_id else event
    the_real_message = technoevent.stringify()
...
async def _(event):
    ...
    technoevent = await event.get_reply_message() if event.reply_to_msg_id else event
    the_real_message = _format.yaml_format(technoevent)
    await eor(event, the_real_message, parse_mode=_format.parse_pre)
