# Made by @TECHNO_K_BOY and @TECHNO_K_BOY
# memify plugin for TechnoUserBot
import asyncio
import base64
import io
import os
...
from Technobot import BOTLOG_CHATID, techno
...
async def maccmd(event):  # sourcery no-metrics
    ...
    technoevent = await event.edit("__Adding frame for media....__")
...
async def memes(event):
    ...
    if ";" in technoinput:
        top, bottom = technoinput.split(";", 1)
    else:
        top = technoinput
        bottom = ""
    ...
    meme = os.path.join("./temp", "technomeme.jpg")
    if gvarstatus("CNG_FONTS") is None:
        CNG_FONTS = "Technobot/helpers/styles/impact.ttf"
    else:
...
async def lang(event):
    ...
    if input_str not in font_list:
        technoevent = await eor(event, "`Give me a correct font name...`")
        await asyncio.sleep(1)
        await technoevent.edit(f"**Available Fonts names are here:-**\n\n{FONTS}")
    else:
        arg = f"Technobot/helpers/styles/{input_str}"
...
async def memes(event):
    ...
    color1 = c_list[0]
    color2 = c_list[1]
    bgcolor = "#080808" if not technoinput else technoinput
...
async def nope(kraken):
    ...
    if troll:
        await kraken.delete()
        techn_ = await troll[0].click(BOTLOG_CHATID)
        if techn_:
...
async def memes(event):
    ...
    technoinput = event.pattern_match.group(1)
    technoinput = 50 if not technoinput else int(technoinput)
    reply = await event.get_reply_message()
    if not reply:
        return await eod(event, "`Reply to supported Media...`")
    san = base64.b64decode("MFdZS2llTVloTjAzWVdNeA==")
    ...
    try:
        await crop(meme_file, outputfile, technoinput)
    except Exception as e:
...
async def memes(event):
    ...
    if not technoinput:
        technoinput = "50"
    if ";" in str(technoinput):
        technoinput, colr = technoinput.split(";", 1)
    else:
        colr = 0
    ...
    try:
        await add_frame(meme_file, outputfile, technoinput, colr)
    except Exception as e:
        return await output[0].edit(f"`{e}`")
    try:
        await event.client.send_file(
