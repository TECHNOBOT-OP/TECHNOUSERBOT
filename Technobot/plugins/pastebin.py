...
async def paste_img(event):
    ...
    d_file_name = None
    technoevent = await eor(event, "`Pasting the text on image`")
...
async def paste_bin(event):
    ...
    technoevent = await eor(event, "`pasting text to paste bin....`")
    ...
    try:
        ...
        await technoevent.edit(result, link_preview=False, parse_mode="html")
    except Exception as e:
        await eod(technoevent, f"**Error while pasting text:**\n`{e}`")
...
async def get_dogbin_content(event):
    ...
    if not url and textx.text:
        ...
        for iurl in urls:
            if (
                ...
                or ("spaceb" in iurl)
                or ("nekobin" in iurl)
                or ("technobin" in iurl)
            ):
                url = iurl
                break
    if not url:
        return await eod(event, "__I can't find any pastebin link.__")
    technoevent = await eor(event, "`Getting Contents of pastebin.....`")
    ...
    if rawurl is None:
        ...
        elif "technobin" in url:
            rawurl = f"http://technobin.up.railway.app/raw/{fid[0]}"
        else:
    ...
    try:
        ...
        return await technoevent.edit(f"**Request timed out.**__{str(TimeoutErr)}__")
    ...
    await eor(technoevent, reply_text)
...
async def _(event):
    ...
    try:
        ...
        return await eod(technoevent, f"**Error while pasting text:**\n`{e}`")
    url = response["url"]
    chat = "@CorsaBot"
    await technoevent.edit("`Making instant view...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await technoevent.edit("```Please unblock me (@CorsaBot) and try```")
        result = ""
        if response:
        ...
        await technoevent.edit(result, link_preview=True)
