...
async def lg1(event):
    ...
    LOGO_FONT_STROKE_WIDTH = gvarstatus("LOGO_FONT_STROKE_WIDTH") or 0
    LOGO_FONT_STROKE_COLOR = gvarstatus("LOGO_FONT_STROKE_COLOR") or None
    fnt = await get_font_file(event.client, "@Techno_Fonts")
    """
    cmd = event.pattern_match.group(1).lower()
    text = event.pattern_match.group(2)
    reply = await event.get_reply_message()
    ...
    if event.reply_to_msg_id:
        ...
        async for i in event.client.iter_messages(
            "@TechnoBot_Logos", filter=InputMessagesFilterPhotos
        ):
            PICS_STR.append(i)
...
async def bad(event):
    ...
    if input_str not in bg_name:
        technoevent = await eor(event, "`Give me a correct background name...`")
        await asyncio.sleep(1)
        await eod(technoevent, lbg_list, time=60)
    else:
...
async def pussy(event):
    ...
    if cmd == "":
        ...
        if input_str not in logo_font:
            ...
            await asyncio.sleep(1)
            await eod(technoevent, font_name, time=80)
        else:
            if " " in input_str:
        ...
        if input_str not in fg_name:
            technoevent = await eor(event, "`Give me a correct color name...`")
            await asyncio.sleep(1)
            await eod(
                technoevent,
                f"**Available color names are here:-**\n\n{fg_list}",
        ...
        techno = re.compile(r"^\-?[1-9][0-9]*\.?[0-9]*")
        isint = re.match(techno, input_str)
