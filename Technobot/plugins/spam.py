...
async def spam_function(event, TECHNO, lol, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(lol[0])
    if len(lol) == 2:
        ...
        for _ in range(counter):
            ...
            if event.reply_to_msg_id:
                await TECHNO.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and TECHNO.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            TECHNO = await event.client.send_file(
                event.chat_id, TECHNO, caption=TECHNO.text
            )
        ...
        if BOTLOG:
            ...
            TECHNO = await event.client.send_file(BOTLOG_CHATID, TECHNO)
            await _technoutils.unsavegif(event, TECHNO)
        return
    elif event.reply_to_msg_id and TECHNO.text:
        spam_message = TECHNO.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
async def spammer(event):
    ...
    TECHNO = await event.get_reply_message()
    ...
    await spam_function(event, TECHNO, lol, sleeptimem, sleeptimet)
...
@techno.techno_cmd(
    ...
    info={
        ...
        "usage": "{tr}cspam <text>",
        "examples": "{tr}cspam Technouserbot",
    },
)
...
@techno.techno_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using TechnoUserBot",
    },
)
async def tmeme(event):
