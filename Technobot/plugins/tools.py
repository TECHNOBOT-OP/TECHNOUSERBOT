...
async def scan(event):
    ...
    chat = "@VS_Robot"
    technoevent = await eor(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        ...
        if response1.text:
            ...
            await eod(technoevent, f"**Please wait for {sec}s before retry**", 15)
        else:
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            if not input_str:
                await eor(technoevent, response3.text[30:])
            else:
...
async def parseqr(event):
    ...
    technoevent = await eor(event, "`Decoding....`")
    ...
    t_response, e_response = (await _technoutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await eor(technoevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await eor(technoevent, f"**Failed to Decode:**\n`{result}`")
...
async def _(event):
    ...
    try:
        ...
        return await technoevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await eod(technoevent, "Created BarCode in {} seconds".format(ms))
...
async def _(event):
    ...
    technoevent = await eor(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
    ...
    if not r.ok:
        return await technoevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
...
Year: {}""".format(
    ...
    await technoevent.edit(output_str, link_preview=True)
