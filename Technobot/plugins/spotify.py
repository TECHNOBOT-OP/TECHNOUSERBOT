...
from Technobot.core.logger import logging
...
from . import BOTLOG, BOTLOG_CHATID, Config, techno, reply_id
...
PATH = "Technobot/cache/spotify_database.json"
        ...
        while SP_DATABASE.SPOTIFY_MODE:
            ...
            if r.status_code == 200:
                ...
                if received["currently_playing_type"] == "track":
                    ...
                    if save_spam("spotify", False):
                        ...
                        await techno.send_message(BOTLOG_CHATID, string)
                else:
            ...
            # need to change anything
                ...
                if save_spam("spotify", False):
                    ...
                    await techno.send_message(BOTLOG_CHATID, stringy)
            ...
            try:
                ...
                full = (await techno(GetFullUserRequest(techno.uid))).full_user
                bio = full.about
                ...
                if to_insert:
                    ...
                    if new_bio:
                        ...
                        if not new_bio == bio:
                            try:
                                await techno(UpdateProfileRequest(about=new_bio))
                    ...
                    if not new_bio:
                        if save_spam("telegram", True):
                            ...
                            await techno.send_message(BOTLOG_CHATID, to_send)
                # not to_insert means no playback
                    ...
                    # back to the original one
                        ...
                        await techno(UpdateProfileRequest(about=old_bio))
...
async def spotifybio(event):
    ...
    if SP_DATABASE.SPOTIFY_MODE:
        SP_DATABASE.SPOTIFY_MODE = False
        if USER_INITIAL_BIO:
            await techno(UpdateProfileRequest(about=USER_INITIAL_BIO["bio"]))
...
def telegraph_lyrics(tittle, artist):
    ...
    try:
        response = telegraph.create_page(
            "Lyrics",
            html_content=result,
            author_name="TechnoBot",
            author_url="https://t.me/Technobot_XD",
        )
        ...
        response = telegraph.create_page(
            ...
            html_content=str(e),
            author_name="TechnoBot",
            author_url="https://t.me/TechnoBot_Official",
        )
    return response["url"], symbol


def file_check():
    logo = "temp/techno_music.png"
    ...
    if not os.path.exists(logo):
        urllib.request.urlretrieve(
            "https://github.com/ITS-LEGENDBOT/RESOURCES/raw/master/Resources/Spotify/TECHNO.png",
            logo,
        )
...
async def spotify_now(event):
    ...
    if link:
        ...
        if not url(link) and "spotify" not in link:
            return await eod(technoevent, "**Give me a correct link...**")
    elif not link:
        if not await sp_var_check(event):
            return
        r = sp_data("https://api.spotify.com/v1/me/player/currently-playing")
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await techno(unblock("DeezerMusicBot"))
            purgeflag = await conv.send_message("/start")
        ...
        await technoevent.delete()
        if cmd == "i":
            songg = await techno.send_file(BOTLOG_CHATID, song)
            fetch_songg = await techno.tgbot.get_messages(BOTLOG_CHATID, ids=songg.id)
            btn_song = await techno.tgbot.send_file(
                BOTLOG_CHATID, fetch_songg, buttons=Button.url("🎧 Spotify", link)
            )
            fetch_btn_song = await techno.get_messages(BOTLOG_CHATID, ids=btn_song.id)
