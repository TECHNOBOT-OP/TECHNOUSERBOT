...
from Technobot import HEROKU_APP, UPSTREAM_REPO_URL, techno
...
lb_info = "https://raw.githubusercontent.com/TECHNOBOT-OP/TECHNOUSERBOT/master/TechnoBoy-info.json"


async def ld_info(lb_info):
    infos = requests.get(lb_info).json()
    _version = infos["TECHNOBOT-INFO"]["version"]
    _release = infos["TECHNOBOT-INFO"]["release-date"]
    _branch = infos["TECHNOBOT-INFO"]["branch"]
    _author = infos["TECHNOBOT-INFO"]["author"]
    _auturl = infos["TECHNOBOT-INFO"]["author-url"]
...
async def update_techno(event, repo, ups_rem, ac_br):
    ...
    TECHNO = await event.edit(
        "`✅ Successfully Updated TechnoBot!\n"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    if heroku_app is None:
        await event.edit(
            f"{txt}\n" "`Invalid Heroku credentials for deploying Technobot dyno.`"
        )
        return repo.__del__()
    TECHNO = await event.edit(
...
async def upstream(event):
    ...
    if ENV and (API_KEY is None or APP_NAME is None):
        return await eor(
            event,
            "`👨‍💻 Set the required vars first to Update the [†ê¢hñðßð†](https://t.me/TechnoBot_Updates/292)`",
        )
    try:
        txt = (
        ...
        if conf is None:
            return await event.edit(
                f"`Unfortunately, the directory {error} does not seem to be a git repository.\nBut we can fix that by force updating the Technobot using .update now.`"
            )
        repo = Repo.init()
    ...
    if force_update:
        await event.edit(
            "`Force-Syncing to latest stable Technobot code, please wait...`"
        )
    if conf == "now":
        await event.edit("`Updating Technobot, please wait....`")
...
async def upstream(event):
    event = await eor(event, "`Pulling the TECHNOBOT repo wait a sec ....`")
    off_repo = "https://github.com/TECHNOBOT-OP/TECHNOBOT"
    os.chdir("/app")
    try:
        txt = (
            "`Oops.. Updater cannot continue due to "
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("`Deploying Technobot, please wait....`")
...
async def variable(var):
    ...
    heroku_var["UPSTREAM_REPO"] = "https://github.com/TECHNOBOT-OP/TECHNOUSERBOT"
