...
def pack_name(userid, pack, is_anim, is_vid):
    if gvarstatus("CUSTOM_STICKER_SETNAME") is not None:
        ...
        return f"TechnoBot_{userid}_{pack}_anim"
    elif is_vid:
        return f"TechnoBot_{userid}_{pack}_vid"
    else:
        return f"TechnoBot_{userid}_{pack}"
    ...
    for message in reqd_sticker_set.documents:
        ...
        if photo:
            ...
            if (
                ...
                async with args.client.conversation("@Stickers") as conv:
                    pack, TECHNOBOTname = await newpacksticker(
                        technoevent,
                        conv,
                        cmd,
                        args,
                ...
                async with args.client.conversation("@Stickers") as conv:
                    pack, TECHNOBOTname = await add_to_pack(
                        technoevent,
                        conv,
                        args,
                        packname,
            ...
            if TECHNOBOTname is None:
                return
            if TECHNOBOTname not in blablapacks:
                blablapacks.append(TECHNOBOTname)
