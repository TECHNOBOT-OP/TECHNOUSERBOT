...
async def grpinfo():
    outstr = "**Plugins in TechnoBot are:**\n\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your TechnoBot are :**\n\n"
...
@techno.techno_cmd(
    ...
    info={
        "header": "To get guide for TechnoBot.",
...
async def _(event):
    ...
    if not input_str:
        ...
        for cmd in cmds:
            outstr += f"  - `{cmdprefix}{cmd}`\n"
        outstr += f"**👨‍💻  Usage : ** `{cmdprefix}help -l <command name>`"
    await eor(event, outstr, aslink=True, linktext="Total Commands of TechnoBot are :")
