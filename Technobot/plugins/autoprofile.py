...
autopic_path = os.path.join(os.getcwd(), "Technobot", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "Technobot", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "Technobot", "photo_pfp.png")
...
async def autopicloop():
    ...
    if AUTOPICSTART and DEFAULT_PIC is None:
        if BOTLOG:
            return await techno.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await techno.upload_file(autophoto_path)
        try:
            await techno(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await techno.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await techno(
                    functions.photos.DeletePhotosRequest(
                        await techno.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await techno(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await techno(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await techno(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await techno(functions.photos.UploadProfilePhotoRequest(file))
        await _technoutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(CHANGE_TIME)
...
techno.loop.create_task(autopfp_start())
techno.loop.create_task(autopicloop())
techno.loop.create_task(digitalpicloop())
techno.loop.create_task(bloom_pfploop())
techno.loop.create_task(autoname_loop())
techno.loop.create_task(autobio_loop())
techno.loop.create_task(custompfploop())
