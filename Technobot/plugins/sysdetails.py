...
async def cpu(event):
    ...
    await eor(
        event, f"**[Techno's](tg://need_update_for_some_feature/) CPU Model:**\n{o}"
    )
...
async def sysdetails(sysd):
    ...
    technoevent = await eor(sysd, "`Fetching system information.`")
    ...
    a, b, c, d = await _technoutils.runcmd(neo)
    result = str(a) + str(b)
    await eor(technoevent, "**Neofetch Result:** `" + result + "`")
