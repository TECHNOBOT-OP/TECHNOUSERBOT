...
@techno.techno_cmd(
    ...
    info={
        ...
        "examples": "{tr}admins @TechnoBot_Support",
    },
)
async def _(event):
...
@techno.techno_cmd(
    ...
    info={
        ...
        "examples": "{tr}bots @TechnoBot_Updates",
    },
)
async def _(event):
    "To get list of bots."
...
async def get_users(show):
    ...
    technoevent = await eor(show, "`getting users list wait...`  ")
    try:
        ...
        mentions += f" {str(err)} \n"
        try:
            await eor(technoevent, mentions)
...
@techno.techno_cmd(
    ...
    info={
        ...
        "examples": "{tr}chatinfo @TechnoBot_Updates",
    },
)
async def info(event):
    "To get group information"
    technoevent = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event, technoevent)
    if chat is None:
        return
    caption = await fetch_info(chat, event)
    try:
        await technoevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
        ...
        await technoevent.edit("`An unexpected error has occurred.`")


async def get_chatinfo(event, technoevent):
    ...
    try:
        ...
        try:
            ...
            await technoevent.edit("`Invalid channel/group`")
            return None
            ...
            await technoevent.edit("`Channel or supergroup doesn't exist`")
            return None
        except (TypeError, ValueError) as err:
            LOGS.info(err)
            await eod(technoevent, "**Error:**\n__Can't fetch the chat__")
            return None
    return chat_info
