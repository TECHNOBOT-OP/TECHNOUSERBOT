...
async def stats(event):  # sourcery no-metrics
    ...
    techno = await eor(event, STAT_INDICATION)
...
async def stats(event):  # sourcery no-metrics
    technocmd = event.pattern_match.group(1)
    technoevent = await eor(event, STAT_INDICATION)
    start_time = time.time()
